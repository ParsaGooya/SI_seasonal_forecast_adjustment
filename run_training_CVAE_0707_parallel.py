...
def train_loop(rank, world_size, train_set, training_runs, params, model_mask, obs_mask, unet, test_year, month, lead_time,  results_dir):
    ...
    net = cVAE(VAE_latent_size = params['VAE_latent_size'], n_channels_x= n_channels_x+ add_feature_dim , sigmoid = sigmoid_activation, NPS_proj = NPSProj, combined_prediction = params['combined_prediction'], VAE_MLP_encoder = params['VAE_MLP_encoder'],
                            scale_factor_channels = params['scale_factor_channels'], skip_VAE_added_dim = params['skip_VAE_added_dim'], saved_deterministic_model = unet, freeze_deterministic = params['freeze_deterministic'],
                              learn_decoder_variance = params['learn_decoder_variance']['status'], noise_injection_std = params['learn_decoder_sampler']['noise_std'], device = rank) 
    ...
    if params['saved_checkpoint_dir'] is not None:
            ...
            if len(glob.glob(saved_checkpoint_dir + '/' + nameSave)) > 0 :
                checkpoint_model_dir = glob.glob(saved_checkpoint_dir+ '/' + nameSave)[0]
            else:
                try:
            ...
            print(f'Restart training from epoch {checkpoint_restart_epoch} ...\n')
            with open(Path(results_dir, "training_parameters.txt"), 'a') as f:
                f.write(f"Restarted training from epoch {checkpoint_restart_epoch} ... \n")
                if params['saved_checkpoint_dir'] != 'same':
                            f.write(f"contintuing {params['saved_checkpoint_dir']} \n")
    ...
    for training in training_runs:
        ...
        if params['version'] == 'IceExtent':
            ...
            if params['low_ress_loss_kernel_size'] is None:  
                # criterion = WeightedMSEKLD(weights=weights, device=rank, weights_mask = weights_mask, hyperparam=1, reduction=params['loss_reduction'], loss_area=loss_region_indices, scale=reg_scale, multi_ress_loss_kernel_size = multi_ress_loss_kernel_size)
                criterion = VAEloss(weights=weights, device=rank, weights_mask = weights_mask,  reduction=params['loss_reduction'], loss_area=None, scale=reg_scale, 
                ...
                # criterion = WeightedMSELowRessKLD(weights=weights, device=rank, weights_mask = weights_mask, hyperparam=1, reduction=params['loss_reduction'], loss_area=loss_region_indices, scale=reg_scale, kernel = params['low_ress_loss_kernel_size'])
                criterion = VAElossLowRess(weights=weights, device=rank, weights_mask = weights_mask, reduction=params['loss_reduction'], loss_area=None, scale=reg_scale,
        ...
        for epoch in tqdm.tqdm(range(epochs)):
            ...
            for batch, (x, y) in enumerate(dataloader):
                ...
                del x, y , m, mu, log_var , cond_mu, cond_log_var 
                gc.collect()
                torch.cuda.empty_cache() 
        ...
        if rank == 0:
            nameSave = results_dir + '/outputs.pt'
            torch.save(outputs, nameSave)
            if test_year*100 + month <= params['ds_raw_ensemble_mean_times'][-1]:
                    nameSave = f"MODEL_V{params['version']}_198101-{test_year * 100 + month -1}_epoch_{epochs + checkpoint_restart_epoch}.pth"
            else:
                if lead_time is not None:
                        nameSave = f"MODEL_final_V{params['version']}_198101-{int(params['ds_raw_ensemble_mean_times'][-lead_time])}_epoch_{epochs + checkpoint_restart_epoch}.pth"
                else:
                        nameSave = f"MODEL_final_V{params['version']}_198101-{int(params['ds_raw_ensemble_mean_times'][-1])}_epoch_{epochs + checkpoint_restart_epoch}.pth"
            ...
            for month in range(1,13,params['forecast_range_months']):
                ...
                if params['path_to_deterministic'] is not None:
                    ...
                    try:
                        ...
                        try:
                            unet.load_state_dict(torch.load(glob.glob(params['path_to_deterministic']+ '/Checkpoints/' +saved_model_year_dir)[0], map_location=torch.device('cpu')))
                        except:
                            print(f'No saved deterministic model found for test year {test_year}')
                            break
                    # unet.to(device) temporarily not input to check whether sending the net to device does automatically take care of this
                else:
                    unet = None
                ...
                # breakpoint()
                mp.spawn(train_loop, args = (world_size, train_set, training_runs, params, model_mask_, obs_mask, unet, test_year, month, lead_time, results_dir) , nprocs = world_size)
                ...
                for training in training_runs:
                        ...
                        ax.set_ylabel('Loss')
                        plt.show()
                        plt.savefig(results_dir+f'/Figures/train_loss_198101-{test_year * 100 + month -1}_{training}_{epochs + checkpoint_restart_epoch}.png')
                        plt.close()
                ...
                if test_year*100 + month <= params['ds_raw_ensemble_mean_times'][-1]:
                    ...
                    net_state_dict = torch.load(results_dir + '/' + nameSave, map_location=torch.device('cpu'))
                    new_state_dict = {}
                    for key, value in net_state_dict.items():
                        if key.startswith('module.'):
                            new_state_dict[key[7:]] = value  # Remove "module." (7 chars)
                    ...
                    if not save:
                        os.remove(results_dir + '/' + nameSave) 
                    else:
                        try:
                    ...
                    if params['version'] == 'IceExtent':   
                        ...
                        criterion_test =  WeightedMSE(weights=weights_, device=0, hyperparam=1, reduction='mean', loss_area=loss_region_indices)
                    ...
                    try:
                        ls_checkpoints  = glob.glob(results_dir + '/Checkpoints/' + f"MODEL_final_*.pth")
                        for link in ls_checkpoints:
                            os.remove(link)
                    except:
                        pass                  
...
if __name__ == "__main__":
    ...
    if params['learn_decoder_sampler']['status']:
        ...
        model_type = model_type + f'-DecoderSamplerV07-{params["learn_decoder_sampler"]["decoder_sample_size"]}'



    if lead_time is None:
        out_dir    = f'{out_dir_x}/N{n_years}_M{lead_months}'
    else:
        out_dir    = f'{out_dir_x}/N{n_years}_LT{lead_time}'
    ...
    if params['lr_scheduler']:
        ...
        params['total_iters'] = 25


    out_dir  = out_dir + f'_{params["subset_dims"]}_lr{params["lr"]}_batch{params["batch_size"]}_e{params["epochs"]}_maskfeautures_equalweights'
