        ...
        for y_idx, test_year in enumerate(test_years):
            ...
            for month in range(1,13,params['forecast_range_months']):
                ...
                if model in [UNet,UNetLCL,UNet2, UNet_NPS, UNet2_NPS, UNet2_small, UNet2_NPS_small]:
                    net = model(n_channels_x= n_channels_x+ add_feature_dim , bilinear = params['bilinear'], sigmoid = sigmoid_activation, skip_conv = params['skip_conv'], combined_prediction = params['combined_prediction'], LocallyConnected = params['LocallyConnected'], clamped = params['clamped'])
                elif model in [ CNN]:
                ...
                for epoch in tqdm.tqdm(range(epochs)):
                    ...
                    if np.mod(epoch + 1 + checkpoint_restart_epoch , 10) == 0:
                            ...
                            plt.show()
                            try:
                                os.remove(glob.glob(results_dir+f'/Figures/train_val_loss_198101-{(test_year - n_validation_years )* 100 + month -1}_epoch_{checkpoint_restart_epoch}_*.png')[0])
                            except:
                                pass
