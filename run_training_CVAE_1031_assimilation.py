...
def run_training(params, n_years, n_validation_years = 0, lead_months = 12, lead_time = None, NPSProj = False,test_years = None,  n_runs=1, results_dir=None, numpy_seed=None, torch_seed=None, save = False):
    ...
    if params['version'] == 2:
        print(' Warning!!! If you used AnomaliesScaler_v*_seasonal or Standardizer as a target preprocessing step make sure to pass "VAE = True" as an argument to the initializer!!!')
    #####################################################################################################
    ...
    if params['fine_tune_on_obs']:
        ...
        obs_in = xr.open_mfdataset(f'/space/hall7/sitestore/eccc/crd/cccma/users/rpg002/data/SI/assimilation/*{crs}*.nc', 
                                combine= 'nested', concat_dim='ensembles')['SICN'].transpose('time','ensembles',...).sortby('ensembles')
        if ensemble_list is not None:
            obs_in = obs_in.sel(ensembles = ensemble_list)
    ...
    for run in range(n_runs):
        ...
        for y_idx, test_year in enumerate(test_years):
            ...
            for month in range(1,13,params['forecast_range_months']):
                ...
                dataloader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=os.cpu_count())
                ...
                dataloader_val = DataLoader(validation_set, batch_size=batch_size, shuffle=False, num_workers=os.cpu_count())                   
