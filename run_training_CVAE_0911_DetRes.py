...
def run_training(params, n_years, n_validation_years = 0, lead_months = 12, lead_time = None, NPSProj = False,test_years = None,  n_runs=1, results_dir=None, numpy_seed=None, torch_seed=None, save = False):
    ...
    assert params['version'] in [1,2,3,1.1]
    ...
    if not params['learn_decoder_sampler']['status']:
        ...
        for y_idx, test_year in enumerate(test_years):
            ...
            for month in range(1,13,params['forecast_range_months']):
                ...
                try:
                    ...
                    assert 'combined' not in params['path_to_deterministic'], 'combined version not supported in DetRes'
                    ...
                    unet = model(n_channels_x= n_channels_x+ add_feature_dim , bilinear = params_saved['bilinear'], sigmoid = sigmoid_activation, skip_conv = params_saved['skip_conv'],  LocallyConnected = LocallyConnected_saved)
                    ...
                    for epoch in tqdm.tqdm(range(epochs)):
                        ...
                        for batch, (x, y) in enumerate(dataloader):
                            ...
                            if params['learn_decoder_variance']['status']:
                                generated_output, output_logvar, deterministic_output, mu, log_var , cond_mu, cond_log_var = net(y, obs_mask, x, model_mask_, sample_size = params['learn_decoder_sampler']["decoder_sample_size"], inject_noise = decoder_inject_noise )
                            else:
                                generated_output, deterministic_output, mu, log_var , cond_mu, cond_log_var = net(y, obs_mask, x, model_mask_, sample_size = params['learn_decoder_sampler']["decoder_sample_size"], inject_noise = decoder_inject_noise )
                                output_logvar = None
                            ...
                            if not decoder_inject_noise:
                                y_ = (y - deterministic_output).unsqueeze(0).expand_as(generated_output) 
                            else:
                                y_ = y - deterministic_output
                        ...
                        del x, y , y_, m, mu, log_var , cond_mu, cond_log_var, loss, MSE, KLD , deterministic_output
                        gc.collect()
                        torch.cuda.empty_cache() 
                        ################################# Validation ###############################
                        net.eval()
                ...
                if test_year*100 + month <= ds_raw_ensemble_mean.time[-1]:
                    ...
                    for i, (x, target) in enumerate(test_set): 
                        ...
                        with torch.no_grad():
                            ...
                            out = net.generation(z, inject_noise = params['learn_decoder_sampler']['status']) 
                            generated_output = net.last_conv(out) + deterministic_output.squeeze() 
                    ...
                    del  test_set , test_raw, test_obs, m, deterministic_output,  generated_output , ds_test, obs_test, cond_mu, cond_var, cond_std
                    gc.collect()
                    torch.cuda.empty_cache() 
...
if __name__ == "__main__":
    ...
    params = {
        "model": cVAE,
        "path_to_deterministic" :  '/space/hall7/sitestore/eccc/crd/cccma/users/rpg002/output/SI/Full/results/NASA/UNet2/run_set_4_convnext/N2_M12_VAL3_F12_v1.1_North_lr0.001_batch100x1_e100_equalweights_1x1_lr_scheduler_bilinear', 
    ...
    if 'low_ress' in params['path_to_deterministic']:
        ...
        out_dir = out_dir + f'_pretrainedUNETlowress{num}_DetRess'
    elif 'multi_ress' in params['path_to_deterministic']:
        num = params['path_to_deterministic'].split('multi_ress_loss')[1]
        out_dir = out_dir + f'_pretrainedUNETmultiress{num}_DetRess'  
    else:
        out_dir = out_dir + '_pretrainedUNET_DetRess'   
