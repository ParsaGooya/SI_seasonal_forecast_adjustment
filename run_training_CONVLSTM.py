...
from models.convlstm import UNetLSTM, PNet, CNNLSTM, UNetLSTM_monthly, CNNLSTM_monthly
...
def run_training(params, n_years, lead_months, lead_time = None, NPSProj = False,test_years = None, n_runs=1, results_dir=None, numpy_seed=None, torch_seed=None, save = False):
    ...
    if params['model'] in [UNet, UNetLCL,UNetLSTM, PNet, UNetLSTM_monthly]:
    ...
    if NPSProj:
        ...
        # if params["model"] == UNet2: REplace with CONVLSTMs for NPS
    ...
    if NPSProj is False:
        if model in [UNet,UNetLCL, CNN, UNetLSTM, PNet, UNetLSTM_monthly]:
        ...
        if model in [RegCNN, CNNLSTM, CNNLSTM_monthly]:
            ...
            for month in range(1,13,params['forecast_range_months']):
                ...
                if test_year*100 + month <= ds_raw_ensemble_mean.time[-1]:
                    if lead_time is None:
                        ...
                        ds_test = ds[n_train - 11:n_train + params['forecast_range_months'],...]
                        obs_test = obs[n_train - 11:n_train + params['forecast_range_months'],...]
                ...
                if params['version'] == 'PatternsOnly':
                        ...
                        del fct_climatology, obs_climatology, target_forecasts, mask_bias_adj
                ...
                if model in [UNetLSTM, PNet]:
                    ...
                    net = model(  n_channels_x= n_channels_x, add_features = add_feature_dim , hidden_dims  =hidden_dims, seq_length = lead_months, kernel_size = kernel_size, decoder_kernel_size = decoder_kernel_size, sigmoid = sigmoid_activation, device =  device)
                elif model in [UNet,UNetLCL,UNet_NPS]:
                    ...
                    net = model(n_channels_x , add_feature_dim , kernel_size = kernel_size, decoder_kernel_size = decoder_kernel_size,DSC = params['DSC'], sigmoid = sigmoid_activation )
                ...
                if lead_time is not None:
                    ...
                    train_set = XArrayDataset(ds_train, obs_train, mask=train_mask,zeros_mask = zeros_mask, in_memory=True, lead_time=lead_time, time_features=time_features,ensemble_features =ensemble_features, aligned = True, month_min_max = month_min_max, model = model.__name__) 
                ...
                if test_year*100 + month <= ds_raw_ensemble_mean.time[-1]:
                    ...
                    ##################################################################################################################################
                    test_years_list = np.arange(1, ds_test[11:].shape[0] + 1)
                    ...
                    if lead_time is None: 
                        ...
                        test_set = ConvLSTMDataset(ds_test, obs_test, lead_time=lead_time,mask = None,zeros_mask = zeros_mask, time_features=time_features,ensemble_features =ensemble_features,  in_memory=False,  month_min_max = month_min_max)
                    if params['version'] == 'IceExtent':   
                    ...
                    if 'ensembles' in ds_test.dims:
                        if lead_time is None:
                            if model in [UNetLSTM,CNNLSTM, UNetLSTM_monthly, CNNLSTM_monthly]:
                                ...
                                test_loss = np.zeros(shape=(ds_test[11:].transpose('time','ensembles','channels',...).sel(lead_time = lead_time).shape[:2]))
                                test_results = np.zeros_like(ds_test[11:].transpose('time','ensembles','channels',...).sel(lead_time = lead_time).data)
                                results_shape = xr.full_like(ds_test[11:].transpose('time','ensembles','channels',...).sel(lead_time = lead_time), fill_value = np.nan)
                                test_time_list =  np.arange(1, results_shape.shape[0] + 1)
                    ...
                    if model in [UNetLSTM,CNNLSTM]:
                        test_results = results_shape.transpose('time','lead_time',...)
                    elif model in [ CNNLSTM_monthly, UNetLSTM_monthly]:  ## PG: if the output is already a map
                    ...
                    if not NPSProj:
                        if model in [UNet, UNetLCL, CNN, UNetLSTM, PNet, UNetLSTM_monthly, CNNLSTM_monthly]:
                                result = reverse_pole_centric(result, subset_dimensions)
                        if model in [RegCNN, CNNLSTM]:
                                result = reverse_segment(result)
                    ##############################################################################################################################################################
                    # Store results as NetCDF   
                    if NPSProj:
                          result = result.rename({'lon':'x', 'lat':'y'})
                          land_mask = land_mask.rename({'lon':'x', 'lat':'y'})
                    ...
                    if params['version'] == 'PatternsOnly':
                        ...
                        if not NPSProj:
                            ...
                            elif model in [RegCNN, CNNLSTM]:
                                result = result * reverse_segment(land_mask)
                        else:
                            result = result * land_mask
...
if __name__ == "__main__":
    ...
    params = {
        "model": UNetLSTM,
        "hidden_dims":  [ 64, 128,128, 64], ## only for SCNN and CNN
