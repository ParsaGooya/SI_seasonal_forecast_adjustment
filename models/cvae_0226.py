...
class cVAE(nn.Module):
    ...
    def __init__( self, VAE_latent_size, n_channels_x=1 ,  sigmoid = True, NPS_proj = False,scale_factor_channels = None, combined_prediction = False, VAE_MLP_encoder = False,skip_VAE_added_dim = False, saved_deterministic_model = None, freeze_deterministic = True, learn_decoder_variance = False, device = torch.device('cpu') ):
        super().__init__()
...
class generation(nn.Module):
    ...
    def __init__( self, VAE_latent_size,  sigmoid = True, VAE_MLP_encoder = False, scale_factor_channels = 4, skip_VAE_added_dim = False  ):
        ...
        self.up2 = Up(128 + self.added_dim , 64, scale_factor_channels = scale_factor_channels)
        self.up3 = Up(64 + self.added_dim , 32, scale_factor_channels = scale_factor_channels)
        self.up4 = Up(32 + self.added_dim , 16, scale_factor_channels = scale_factor_channels)
            # self last layer:	
    def forward(self, z):
        # Upsampling
        x = self.combine(z)
        ...
        if self.added_dim >0:
            z_ = self.skip1(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,12,22))
            x = torch.cat([x,z_], dim = 1)
        x = self.up2(x, pad = (0,1,0,1))  # (batch, 64, 25, 45)
        if self.added_dim >0:
            z_ = self.skip2(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,25,45))
            x = torch.cat([x,z_], dim = 1)
        x = self.up3(x)  # (batch, 32, 50, 90)
        if self.added_dim >0:
            z_ = self.skip3(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,50,90))
            x = torch.cat([x,z_], dim = 1)
...
class generation_NPS(nn.Module):
    ...
    def __init__( self, VAE_latent_size,   sigmoid = True, VAE_MLP_encoder = False , scale_factor_channels = 4 , skip_VAE_added_dim = False ):
        ...
        self.added_dim = 0
        if VAE_MLP_encoder:
            self.combine = nn.Linear(VAE_latent_size, 512 * 13*9)
        else:
            self.combine = nn.Conv2d(VAE_latent_size, 512, kernel_size=1)
            assert skip_VAE_added_dim is False
        if skip_VAE_added_dim :
            self.skip1 = nn.Linear(VAE_latent_size, 1 * 27*19)
        ...
        self.up2 = Up(256+ self.added_dim , 128, scale_factor_channels = scale_factor_channels)
        self.up3 = Up(128+ self.added_dim , 64, scale_factor_channels = scale_factor_channels)
        self.up4 = Up(64+ self.added_dim , 32, scale_factor_channels = scale_factor_channels)
        self.up5 = Up(32+ self.added_dim , 16, scale_factor_channels = scale_factor_channels)

            # self last layer:	
    def forward(self, z):
        x = self.combine(z)
        if self.VAE_MLP_encoder:
            x = torch.unflatten(x, dim = 1, sizes = (512,13,9))
        # Upsampling
        x = self.up1(x, pad = (0,1,0,1))  # (batch, 256, 27, 27)
        if self.added_dim >0:
            z_ = self.skip1(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,27,19))
            x = torch.cat([x,z_], dim = 1)
        x = self.up2(x)  # (batch, 128, 54, 54)
        if self.added_dim >0:
            z_ = self.skip2(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,54,38))
            x = torch.cat([x,z_], dim = 1)
        x = self.up3(x)  # (batch, 64, 108, 108)
        if self.added_dim >0:
            z_ = self.skip3(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,108,76))
            x = torch.cat([x,z_], dim = 1)
        x = self.up4(x)  # (batch, 32, 216, 216)
        if self.added_dim >0:
            z_ = self.skip4(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,216,152))
            x = torch.cat([x,z_], dim = 1)
        x = self.up5(x)  # (batch, 16, 432, 432)     
        if self.added_dim >0:
            z_ = self.skip5(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,432,304))
...
class prior_recognition_NPS(nn.Module):
    ...
    def __init__( self,  n_channels_x=1 ,  sigmoid = True, VAE_latent_size = None, VAE_MLP_encoder = False, scale_factor_channels = 4 ):
        ...
        if VAE_latent_size is None:
              VAE_latent_size = 512
        if VAE_MLP_encoder:
            self.VAE_MLP_input_dim = 13 * 9 * 512
        else:
            self.VAE_MLP_input_dim = None
        if scale_factor_channels is None:
             mid_channels = None
    ...
    # Downsampling
        ...
        x6, mask6  = self.d5(x5, mask5)  # (batch, 512, 13, 9)
        
        mu, log_var = self.last_conv_down(x6, mask6)  # (batch, 512, 13, 9)     
        return mu, log_var
...
class prediction_NPS(nn.Module):
    ...
    # Downsampling
        ...
        x7 = self.last_conv_down(x6, mask6)  # (batch, 1024, 13, 9)

        # Upsampling
        x = self.up1(x7, pad = (0,1,0,1))  # (batch, 512, 27, 19)
        x = self.up2(x)  # (batch, 256, 54, 38)
        x = self.up3(x)  # (batch, 128, 108, 76)
        x = self.up4(x)  # (batch, 64, 216, 152)
        x = self.up5(x)  # (batch, 32, 432, 304)
        if self.output_layer:
            x = self.last_conv(x)
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, up_kernel = 3, scale_factor_channels = 4):
            super().__init__()
            if scale_factor_channels is None:
                mid_channels = None
            else:
            ...
            self.conv = DoubleConvNext(in_channels, out_channels, mid_channels= mid_channels, multi_channel=False, return_mask=False)
...
# class cVAE_res2(nn.Module):  ############### !!!!!!!!!!!!!!!!!!! needs editing for reading the variance learned model output  !!!!!!!!!!!!!!!!!!!!!!!!!  ############################
	
#     def __init__( self, VAE_latent_size, n_channels_x=1 ,  sigmoid = True, NPS_proj = False,scale_factor_channels = None, combined_prediction = False, VAE_MLP_encoder = False,skip_VAE_added_dim = False, saved_deterministic_model = None, freeze_deterministic = True, learn_decoder_residual = False, device = 'cpu' ):
#         super().__init__()
#         self.combined_prediction = combined_prediction
#         self.VAE_MLP_encoder = VAE_MLP_encoder
#         self.n_channels_x = n_channels_x
#         self.loaded_unet = True if saved_deterministic_model is not None else False
#         self.learn_decoder_residual = learn_decoder_residual
...
#         generated_output = self.last_conv(out)
#         generated_output = torch.unflatten(generated_output, dim = 0 , sizes = out_shape[0:2])
#         if self.learn_decoder_residual:
        
#             feetures = torch.cat([model[0], model[1]], dim=1)[:,1:] if (type(model) == list) or (type(model) == tuple) else None
#             if feetures is not None:
#                 feetures = feetures.unsqueeze(0).expand(sample_size, *feetures.shape)
#                 inp = torch.cat([generated_output, feetures], dim = -3)
#                 del feetures
#             else:
#                 inp = generated_output
#             inp = torch.flatten(inp, start_dim = 0, end_dim = 1)
#             output_residual = self.residual(inp, obs_mask.expand(inp.shape[-3:]))
#             output_residual = torch.unflatten(output_residual, dim = 0 , sizes = out_shape[0:2])
#             del inp
