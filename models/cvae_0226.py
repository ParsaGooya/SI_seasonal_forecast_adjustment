...
class generation_NPS(nn.Module):
    ...
    def __init__( self, VAE_latent_size,   sigmoid = True, VAE_MLP_encoder = False , scale_factor_channels = 4 , skip_VAE_added_dim = False ):
        ...
        if VAE_MLP_encoder:
            ...
            assert skip_VAE_added_dim is False
        if skip_VAE_added_dim :
            self.skip1 = nn.Linear(VAE_latent_size, 1 * 27*27)
            self.skip2 = nn.Linear(VAE_latent_size, 1 *54*54)
    ...
    def forward(self, z):
        ...
        if self.added_dim >0:
            z_ = self.skip1(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,27,27))
            x = torch.cat([x,z_], dim = 1)
        x = self.up2(x)  # (batch, 128, 54, 54)
        if self.added_dim >0:
            z_ = self.skip2(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,54,54))
            x = torch.cat([x,z_], dim = 1)
        x = self.up3(x)  # (batch, 64, 108, 108)
        if self.added_dim >0:
            z_ = self.skip3(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,108,108))
            x = torch.cat([x,z_], dim = 1)
        x = self.up4(x)  # (batch, 32, 216, 216)
        if self.added_dim >0:
            z_ = self.skip4(z)
            z_ = torch.unflatten(z_, dim = 1, sizes = (self.added_dim,216,216))
            x = torch.cat([x,z_], dim = 1)
...
class prediction(nn.Module):
    ...
    def __init__( self,  n_channels_x=1 ,  sigmoid = True , scale_factor_channels = 4):
