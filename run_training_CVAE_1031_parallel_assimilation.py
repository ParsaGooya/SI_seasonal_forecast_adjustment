...
def train_loop(rank, world_size, train_set, validation_set, training_runs, params, model_mask, obs_mask, unet, test_year, month, lead_time,  results_dir):
    ...
    net = model(VAE_latent_size = params['VAE_latent_size'], n_channels_x= n_channels_x+ add_feature_dim , sigmoid = sigmoid_activation, NPS_proj = NPSProj, combined_prediction = params['combined_prediction'], VAE_MLP_encoder = params['VAE_MLP_encoder'],
                            scale_factor_channels = params['scale_factor_channels'], skip_VAE_added_dim = params['skip_VAE_added_dim'], saved_deterministic_model = unet, freeze_deterministic = params['freeze_deterministic'], clamped = params['clamped'],
                              learn_decoder_variance = params['learn_decoder_variance']['status'], noise_injection_std = params['learn_decoder_sampler']['noise_std'], noise_injection_level =  params['learn_decoder_sampler']['noise_injection_level'], LocallyConnected= params['LocallyConnected'], device = rank) 
    ...
    for training in training_runs:
                ...
                if epoch_MSE_validation[-1] < best_valScore - ( 0.02 * best_valScore):  # if new score not 5% better than best val score
                    ...
                    if params['earlystoppingbuffer'] is not None:
                        ...
                        if (earlystopping_counter >= params['earlystoppingbuffer']) and (epoch >= 15 ):         
                            ...
                            print(f"epoch val {loss_function} score {epoch_MSE_validation[-1]} has not decreased over {params['earlystoppingbuffer']} epochs compared to best {best_valScore} ")
                            with open(Path(results_dir, "training_parameters.txt"), 'a') as f:
        ...
        data_to_gather[training]['best_valScore_GCN'] = best_valScore_GCN
...
def run_training(params, n_years, n_validation_years = 0, lead_months = 12, lead_time = None, NPSProj = False,test_years = None,  n_runs=1, results_dir=None, numpy_seed=None, torch_seed=None, save = False):
    ...
    if params['version'] == 2:
        ...
        'Make sure the preprocessing functions are working properly for assimilation targets. \n' +
        '=======================================================')
    ...
    if params['fine_tune_on_obs']:
        ...
        obs_in = xr.open_mfdataset('/space/hall7/sitestore/eccc/crd/cccma/users/rpg002/data/SI/assimilation/*.nc', 
                               combine= 'nested', concat_dim='ensembles')['SIC'].transpose('time','ensembles',...)
    ...
    if not NPSProj:
        ...
        if not params['fine_tune_on_obs']:
            obs_in = obs_in.where(obs_in<1000,np.nan)
        if 'ensemble_error' in params['time_features']:
    ...
    if 'ensembles' not in obs_in.dims:
        land_mask = obs_in.mean('time').where(np.isnan(obs_in.mean('time')),1).fillna(0)
    else:
        land_mask = obs_in.isel(ensembles = 0).mean('time').where(np.isnan(obs_in.isel(ensembles = 0).mean('time')),1).fillna(0).drop('ensembles')
    ...
    if any([params['active_grid'],'active_mask' in params["time_features"], 'full_ice_mask' in params["time_features"]]):
        if 'ensembles'  in obs_raw.dims:
            zeros_mask_full = xr.concat([zeros_mask_gen(obs_raw.isel(ensembles = 0).isel(lead_time = 0).drop('lead_time').where(obs_raw.time<(test_year - n_validation_years) *100, drop = True ), 3) for test_year in test_years], dim = 'test_year').assign_coords(test_year = test_years)
        else:
    ...
    for run in range(n_runs):
        ...
        for y_idx, test_year in enumerate(test_years):
            ...
            for month in range(1,13,params['forecast_range_months']):
                ...
                if 'ensembles' in obs_raw.dims: 
                    preprocessing_mask_obs = np.broadcast_to(train_mask[:n_train - month + 1,...,None, None,None,None], obs_baseline.shape)
                else:
                ...
                if params['saved_checkpoint_dir'] != 'None':
                    ...
                    if params['fine_tune_on_obs']:
                        saved_checkpoint_dir = saved_checkpoint_dir.split('_Fine_Tuned_from_Assim')[0]
                ...
                for training in training_runs:
                        ...
                        for gpu_ind in range(world_size):
                            if gpu_ind == 0:
                                ...
                                best_valScore_GCN =  (outputs[0][training]['best_valScore_GCN'])
                ...
                if test_year*100 + month <= params['ds_raw_ensemble_mean_times'][-1]:
                    ...
                    for i, (x, target) in enumerate(test_set): 
                        ...
                        with torch.no_grad():
                            ...
                            del x, target
                            if params['learn_decoder_variance']['status']:
                                _, _, deterministic_output, mu, log_var, cond_mu, cond_log_var = net(test_obs, obs_mask, test_raw, model_mask_, sample_size = 1, inject_noise = params['learn_decoder_sampler']['status'] )
                            else:
                                _, deterministic_output, mu, log_var, cond_mu, cond_log_var = net(test_obs, obs_mask, test_raw, model_mask_, sample_size = 1, inject_noise = params['learn_decoder_sampler']['status'] )
                            ...
                            if all([cond_mu is not None, cond_log_var is not None]):
                                ...
                                z =  Normal(cond_mu, cond_std).rsample(sample_shape=(params['BVAE'],)).squeeze().to(0)    ### should squeeze be there?
                                # z = torch.unflatten(z , dim = -1, sizes = cond_std.shape[-3:])
                            else:
                                z =  Normal(torch.zeros_like(mu), torch.ones_like(log_var)).rsample(sample_shape=(params['BVAE'],)).squeeze().to(0)    ### should squeeze be there?
                            ...
                            if params['learn_decoder_variance']['status']:
                                epsilon = torch_normal_sampling(torch.zeros(generated_output.shape).to(generated_output), torch.sqrt(torch.exp(output_logvar) + 1e-4), num_samples=  1 ).to(0)[0]
                    ...
                    del  test_set , test_raw, test_obs, m, deterministic_output,  generated_output , ds_test, obs_test, cond_mu, cond_var, cond_std, basic_unet, mu, log_var
                    gc.collect()
                    torch.cuda.empty_cache() 
...
if __name__ == "__main__":
    ...
    params['beta'] =  0.01 #dict(start = 0, end =0.01, num_epoch_warmup = 10,  num_epochs_hold = None) #params['epochs'])  

    obs_ref = 'NASA'
    NPSProj = False
    
    out_dir_x  = f'/space/hall7/sitestore/eccc/crd/cccma/users/rpg002/output/SI/Full/results/{obs_ref}/{params["model"].__name__}/run_set_3_convnext_assimilation'
    ...
    if params['fine_tune_on_obs']:
        assert params['saved_checkpoint_dir'] is not None
    ...
    if params['clamped']:
        out_dir = out_dir + '_clamped'

    if params['saved_checkpoint_dir'].lower() != 'same':
        if params['saved_checkpoint_dir'] != 'None':
            if params['fine_tune_on_obs']:
                out_dir = out_dir + '_Fine_Tuned_from_Assim'
            else: 
